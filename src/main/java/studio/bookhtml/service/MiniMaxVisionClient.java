...
public class MiniMaxVisionClient {
    ...
    public List<Block> assist(byte[] png,List<Block> qwenLines,String layout,BooleanSupplier cancelled)throws OcrException{
        ...
        try{List<Map<String,Object>> sources=new ArrayList<>();for(int i=0;i<qwenLines.size();i++){Block b=qwenLines.get(i);sources.add(Map.of("id",b.id(),"readingOrder",i,"text",b.original(),"bbox",b.bbox()));}String prompt="你只做版面结构分析，不转录、不改写、不删除 Qwen 文字。依据图片和 sourceLines 返回严格 JSON：{\"blocks\":[{\"id\":\"b1\",\"type\":\"text|heading|figure|table|caption|page-number|formula\",\"order\":0,\"bbox\":{\"x\":0.1,\"y\":0.1,\"width\":0.2,\"height\":0.2},\"writingMode\":\"vertical-rl|horizontal-tb\",\"sourceIds\":[\"qwen-line-1\"],\"suggestion\":\"仅可选校对建议\",\"uncertain\":true}]}。figure/table/formula 的 bbox 必须是相对完整图片的 0..1 对象，width/height 是长度，绝不是右下角坐标；文字块 bbox 由服务端按 sourceIds 计算，你不得改写来源文字框。文字块必须只引用给定 ID，并让 sourceIds 严格按真实阅读顺序排列；严格依据 readingOrder 和图片判断版面，不得按 ID 数字排列。不可在 suggestion 外输出修正文。竖排双页先右页再左页，同页先右栏再左栏；不得跨插图合并段落。图表块可无 sourceIds。不得重复引用 ID。只有存在真实疑点才设置 uncertain=true 并给出 suggestion，否则 uncertain=false 且不输出 suggestion。版式偏好="+layout+"。sourceLines="+json.writeValueAsString(sources);
            ...
            HttpRequest request=HttpRequest.newBuilder(URI.create(config.minimaxBaseUrl().replaceAll("/+$","")+"/chat/completions")).timeout(Duration.ofSeconds(config.minimaxTimeoutSeconds())).header("Authorization","Bearer "+config.minimaxApiKey()).header("Content-Type","application/json").POST(HttpRequest.BodyPublishers.ofString(json.writeValueAsString(body))).build();HttpResponse<String>response=null;
            for(int attempt=0;attempt<3;attempt++){if(cancelled.getAsBoolean())throw new CancelledException();response=transport.send(request);if(response.statusCode()!=429)break;try{Thread.sleep(250L*(1L<<attempt));}catch(InterruptedException e){Thread.currentThread().interrupt();throw new CancelledException();}}
            if(response==null||response.statusCode()==429)throw new OcrException("MiniMax 请求频率受限，请稍后重试");if(response.statusCode()<200||response.statusCode()>=300)throw new OcrException("MiniMax 辅助失败（HTTP "+response.statusCode()+"）");JsonNode root=json.readTree(response.body());if(root.has("error")||root.path("base_resp").path("status_code").asInt(0)!=0)throw new OcrException("MiniMax 返回业务错误");JsonNode choice=root.at("/choices/0");if("length".equalsIgnoreCase(choice.path("finish_reason").asText()))throw new OcrException("MiniMax 结构输出被截断");JsonNode content=choice.at("/message/content");if(!content.isTextual())throw new OcrException("MiniMax 返回结构无效");return merge(stripFence(content.asText()),qwenLines);
        }catch(ApiException|CancelledException|OcrException e){throw e;}catch(Exception e){throw new OcrException("MiniMax 辅助请求失败",e);}
    }
