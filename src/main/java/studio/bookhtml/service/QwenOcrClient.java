...
public class QwenOcrClient {
    ...
    List<Block> parse(String body,int width,int height,String requestedLayout)throws OcrException{
        try{JsonNode root=json.readTree(body);JsonNode choice=root.at("/output/choices/0");if("length".equalsIgnoreCase(choice.path("finish_reason").asText()))throw new OcrException("Qwen OCR 输出被截断");List<JsonNode>wordGroups=new ArrayList<>();findFields(choice,"words_info",wordGroups);if(wordGroups.isEmpty())throw new OcrException("Qwen OCR 返回缺少 words_info");List<RawLine>lines=new ArrayList<>();int seq=0;
            ...
            if(lines.isEmpty()){if(wordGroups.stream().allMatch(n->n.isArray()&&n.isEmpty()))throw new OcrNoTextException("Qwen OCR 未检出文字");throw new OcrException("Qwen OCR 未返回可用文字坐标");}String mode=chooseMode(lines,requestedLayout);Comparator<RawLine>order="vertical-rl".equals(mode)?Comparator.<RawLine>comparingDouble(x->-x.bbox[0]).thenComparingDouble(x->x.bbox[1]):Comparator.<RawLine>comparingDouble(x->x.bbox[1]).thenComparingDouble(x->x.bbox[0]);lines.sort(order);List<Block>result=new ArrayList<>();int index=0;for(RawLine l:lines){String id="qwen-line-"+(index+1);result.add(new Block(id,"text",index++,l.bbox,mode,l.text,l.text,null,true,false,null,"qwen",List.of(id),null,l.raw));}BlockValidator.validate(result);return result;
