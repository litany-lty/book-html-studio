...
public class PageProcessor {
        ...
        if(nativeBlocks.isPresent()&&!nativeBlocks.get().isEmpty()&&!preferOcrOverNative(store.pdf(bookId),pageNumber,nativeBlocks.get(),warnings,cancelled)){
            ...
            try{
                if(PaddleOcrPipeline.isPaddle(provider)){
                    ...
                    else if(assist){QwenTocRecoveryService.RecoveryResult recovery=tocRecovery.recover(image,blocks,cancelled);if(recovery.warning()!=null)warnings.add(recovery.warning());if(recovery.recovered()){blocks=guardedAssist(sourceRecords,recovery.blocks(),warnings,"目录恢复来源校验失败，已保留"+ocrLabel+"原始结果");actualProvider=paddleResult.usedProvider()+"+qwen-toc-recovery";}else if(recovery.attempted())blocks=guardedAssist(sourceRecords,recovery.blocks(),warnings,"目录恢复来源校验失败，已保留"+ocrLabel+"原始结果");else if(shouldRunLayoutAssist(recovery)&&qwenLayout.configured()){try{List<Block> assisted=qwenLayout.assist(qwenOcr.encodeWithin(image).bytes(),blocks,layout,cancelled);blocks=guardedAssist(sourceRecords,assisted,warnings,"Qwen3.8-Max 结构辅助来源校验失败，已保留"+ocrLabel+"原始结果");actualProvider=paddleResult.usedProvider()+"+qwen-assist";}catch(CancelledException e){throw e;}catch(Exception e){String detail=JobService.safeDetail(e);warnings.add("Qwen3.8-Max 结构辅助失败，本页已保留"+ocrLabel+"原始结果"+(detail==null?"":"："+detail));blocks=sourceRecords;}}else warnings.add("Qwen3.8-Max 结构辅助未配置，本页仅保留"+ocrLabel+"结果");}
    ...
    /** 阶段3：来源门禁——辅助结果丢失/重复/新增一律回退原始并警告，不悄悄交付。 */
    private List<Block> guardedAssist(List<Block> source,List<Block> assisted,List<String> warnings,String message){
        if(assisted==null||QualityGate.hasSourceLossOrDuplication(source,assisted)){warnings.add(message);return source;}
        return assisted;
    }
