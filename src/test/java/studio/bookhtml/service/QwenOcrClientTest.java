...
class QwenOcrClientTest {
    ...
    @Test void retries429OnlyThreeTimes()throws Exception{@SuppressWarnings("unchecked")HttpResponse<String>response=mock(HttpResponse.class);when(response.statusCode()).thenReturn(429);AtomicInteger calls=new AtomicInteger();QwenOcrClient client=new QwenOcrClient(TestConfigs.config(temp,"key",""),new ObjectMapper(),r->{calls.incrementAndGet();return response;});assertThrows(OcrException.class,()->client.recognize(new byte[1],200,200,"auto",()->false));assertEquals(3,calls.get());}
