...
def main():
    ...
    try:
        ...
        check("默认阅读模式", evaluate("document.body.dataset.readerMode==='reading' && getComputedStyle(document.querySelector('#review-panel')).display==='none'"))
        evaluate("document.querySelector('#proof-toggle').click()")
        check("显式进入校对模式", wait("document.body.dataset.readerMode==='proof' && getComputedStyle(document.querySelector('#review-panel')).display!=='none'"))
