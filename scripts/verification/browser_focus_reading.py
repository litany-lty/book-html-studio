...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            check('task_panel_is_not_in_the_reading_flow', page.locator('#reading-window-dialog #job-panel').count() == 1 and page.locator('#job-panel').is_hidden())
            check('duplicate_current_page_buttons_removed', page.locator('#retry-page-header,#reload-page-header,#comprehensibility-check-btn').count() == 0)
            check('whole_book_processing_is_explicit', not page.locator('#auto-process-all').is_checked() and not page.locator('#reading-window-auto-all').is_checked())
            page.screenshot(path=str(OUT/'desktop-reading-clean.png'))
            page.click('#reading-window-open')
            check('task_controls_remain_accessible', page.locator('#job-panel').is_visible() and page.locator('#job-toggle').is_visible())
            page.click('#job-toggle')
            check('automatic_check_does_not_require_its_own_setting', page.locator('#job-form').is_visible() and '自动可理解性自检默认运行' in page.locator('#qwen-assist-note').inner_text())
